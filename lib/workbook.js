...
class XlsxStreamReaderWorkBook extends Stream {
  constructor(options) {
    super();
  ...
  _parseXML(entryStream, entryHandler, endHandler) {
    ...
    var parser = Sax.createStream(this.options.saxStrict, saxOptions);

    entryStream.on("end", node => {
  ...
  _parseSharedStrings(nodeData) {
    ...
    if (nodeObjName && nodeObjName.name === "t") {
      ...
      if (
        nodeObjValue &&
        typeof nodeObjValue === "object" &&
        nodeObjValue.hasOwnProperty("name") &&
        nodeObjValue.name === "t"
      ) {
  ...
  _parseWorkBookInfo(nodeData) {
    nodeData.forEach(data => {
      if (data.name === "sheet") {
        this.workBookInfo.sheetRelationshipsNames[data.attributes["r:id"]] =
  ...
  _parseWorkBookRels(nodeData) {
    nodeData.forEach(data => {
      if (data.name === "Relationship") {
        this.workBookInfo.sheetRelationships[data.attributes.Target] =
          data.attributes.Id;
      }
    });
  }
  ...
  _getSheetName(sheetPath) {
    return this.workBookInfo.sheetRelationshipsNames[
      this.workBookInfo.sheetRelationships[sheetPath]
    ];
  }
}
